...
class Road {
  constructor({ laneChangeDelayTicks, timeOnRoadFunction, numberOfLanes }, metricsCalculator) {
  ...
  conditionallyChangeLanes(isChangingLanes) {
    ...
    this.lanes.forEach((cars, laneIndex) =>
      carsToSwitch[laneIndex] = cars.filter(car => {
        ...
        return isChangingLanes(car, this.lanes[shorterLaneIndex].length);
      }));
