...
class Road {
  ...
  conditionallyChangeLanes(isMetLaneChangeCondition) {
    let carsToSwitch = newArray2D(this.lanes.length);
  ...
  changeLanes(carsToSwitch) {
    ...
    carsToSwitch.forEach((cars, laneIndex) => {
      cars.forEach(car => {
        ...
        this.lanes[laneIndex].splice(carIndex, 1); // remove from original lane

        car.timeInLane = 0; // reset timeInLane counter
        this.lanes[this.shortestAdjacentLaneIndexFor(laneIndex)].push(car); // add to new lane
      });
    });
  }
