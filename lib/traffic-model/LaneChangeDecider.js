...
class LaneChangeDecider {
  constructor({
    laneChangeDelayTicks = 0, minTicksInLane, minTickDelayPercent, minTickSpeedupPercent,
  }) {
  ...
  isExceededMinTimeSaved(car, shorterLaneLength) {
    if (!isSet(this.minTickSpeedupPercent)) return true;

    return car.timeSpeedupPercent(shorterLaneLength, this.laneChangeDelayTicks)
      >= this.minTickSpeedupPercent;
  }

  isExceededMinLaneDelay(car) {
    if (!isSet(this.minTickDelayPercent)) return true;

    return car.timeDelayPercent() >= this.minTickDelayPercent;
  }

  isExceededMinTimeInLane(car) {
    if (!isSet(this.minTicksInLane)) return true;

    return car.elapsedTimeInLane >= this.minTicksInLane;
  }
}
