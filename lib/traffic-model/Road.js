...
const { arrayDifference, newArray2D } = require('../utils');
...
class Road {
    ...
    this.lanes.forEach((cars, laneIndex) => {
      ...
      while (!isDoneAdjustingLanes) {
        ...
        if (newlyExitingCars.length > 0) {
          ...
          cars.forEach(car => car.reCalculateTotalTime(cars.length));
        } else {
  ...
  conditionallyChangeLanes() {
    ...
    this.lanes.forEach((cars, laneIndex) => carsToSwitch[laneIndex] = cars.filter((car) => {
