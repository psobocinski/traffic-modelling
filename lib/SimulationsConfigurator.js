...
const inquirer = require('inquirer'),
  ...
  { createConfigFile } = require('./utils/fileCreators');

function processAnswers({independentVariable, smallestValue, increment, numberOfSimulations}) {
...
function buildSimulationsConfig(independentVariable, smallestValue, increment, numberOfSimulations) {
  return Array(numberOfSimulations).fill().map((_, index) =>
    buildSimulationConfig(independentVariable, smallestValue + increment * index));
}
