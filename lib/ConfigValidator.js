...
function validateGreaterThanOrEqualTo(minimum, number) {
  if (parseFloat(number) < minimum) return `${number} must be greater than or equal to ${minimum}`;
}

function validateGreaterThan(minimum, number) {
  if (parseFloat(number) <= minimum) return `${number} must be greater than ${minimum}`;
}
