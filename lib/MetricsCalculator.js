...
class MetricsCalculator {
  constructor({ output, laneChangeConditions, laneChangeDelayTicks, lanePlacement }, arrivalSequence) {
    this.columnData = Object.keys(output.columns);
  ...
  results() {
    return this.columnData.map(datum => this[datum]());
  }

  minTicksInLane() {
    return this.laneChangeConditions.minTicksInLane;
  }

  minTickSpeedupPercent() {
    return this.laneChangeConditions.minTickSpeedupPercent;
  }
  ...
  printSimulationInfo() {
    ...
    console.log('Lane Change Conditions', this.laneChangeConditions);
  }
